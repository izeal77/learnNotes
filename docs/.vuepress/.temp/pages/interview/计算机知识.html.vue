...
<p>get：请求服务器发送某些资源</p>
<p>head：请求资源的头部信息，用来在下载一个大文件之前先获取大小再决定是否要下载</p>
<p>options：获取目的资源所支持的通信选项</p>
<p>post：发送数据给服务器</p>
<p>put：新增资源或者使用请求中的有效负载替换目标资源的表现形式（更新资源）</p>
<p>delete：删除资源</p>
<p>patch：对资源进行部分修改</p>
<p>connect：预留给能够将连接改为管道方式的代理服务器</p>
<p>trace：回显服务器收到的请求，用于测试或者诊断</p>
<h4 id="get和post" tabindex="-1"><a class="header-anchor" href="#get和post" aria-hidden="true">#</a> <strong>get和post</strong></h4>
<p>传输方式：get通过url传输数据，post通过请求体传输。</p>
<p>安全性：post更安全，get的数据在url中，可以通过历史记录和缓存查到，不安全</p>
<p>数据类型：get只允许ASCII字符，post无限制</p>
<p>get无害性：浏览器操作get请求是无害的，post可能重复提交表单</p>
<p>特性：get不会引起服务器状态变化（安全）且幂等，post是非安全非幂等</p>
<h4 id="post和put" tabindex="-1"><a class="header-anchor" href="#post和put" aria-hidden="true">#</a> <strong>post和put</strong>：</h4>
<p>put：幂等，连续调用多次效果一样，以最新的一次调用为准，post是非幂等的</p>
<p>put指向单一资源，post指向资源集合，用来新增。</p>
<p><strong>put和patch</strong>：都是更新资源，patch用来对已知资源进行局部更新</p>
<h4 id="状态码" tabindex="-1"><a class="header-anchor" href="#状态码" aria-hidden="true">#</a> 状态码</h4>
<p>200 OK,表示从客户端发来的请求在服务器端被正确的处理</p>
<p>302 found,临时性重定向，表示资源临时被分配了新的url</p>
<p>304 not modified，上次请求之后请求的资源未被修改</p>
<p>400 bad request 请求报文存在语法错误</p>
<p>401 unauthorized 表示发送的请求需要有通过HTTP认证的认证信息</p>
<p>403 forbidden 对请求资源的访问被服务器拒绝（token过期、爬虫没有权限）</p>
<p>404 not found 在服务器上没找到请求的资源</p>
<p>500 internal sever error 服务器端在执行请求的时候发生了错误</p>
<p><strong>307 303 302区别</strong></p>
<p>在http1.1的时候为了细化302 分为了303和307</p>
<p>303明确表示客户端应当采用get方法获取资源，他会把post请求变为get进行重定向，307会遵守标准</p>
<h4 id="http的keep-alive" tabindex="-1"><a class="header-anchor" href="#http的keep-alive" aria-hidden="true">#</a> HTTP的keep-alive</h4>
<p>在http1.0的时候，每次http请求都要创建一个连接，而创建连接的过程需要消耗资源和时间，为了减少资源消耗，缩短响应的时间，就需要重用连接。在http1.1的时候在http请求头中加入了connection：keep-alive来告诉对方这个请求响应完成后不要关闭，以此保持长连接。</p>
...
<p>使用HTTPS,HTTPS基于HTTP协议，不过采用了SSL（安全套接层）协议或者TLS（安全层传输协议）对HTTP协议传输的数据进行加密，还提供了一种校验机制，信息一旦被篡改，通信双方就会立刻发现。</p>
...
<h4 id="http1-0和http1-1的区别" tabindex="-1"><a class="header-anchor" href="#http1-0和http1-1的区别" aria-hidden="true">#</a> HTTP1.0和HTTP1.1的区别</h4>
<p>HTTP1.1默认支持长连接。HTTP1.0需要使用keep-alive参数告知服务器端要建立一个长连接</p>
<p>节约带宽：HTTP1.1支持只发送header信息，如果服务器认为客户端有权限请求服务器，就返回100，否则返回401，接收到100的时候才开始把请求body发送到服务器，当为401就不发送请求body，节约带宽。另外HTTP还支持传送内容的一部分，这是断点续传的基础。这是1.1的</p>
<p>HOST域：web server上的多个虚拟站点可以共享同一个ip和端口（在HTTP1.1中实现）</p>
...
<p>http1会在请求和响应的时候重复的携带不常改变的、冗长的头部数据，给网络带来额外负担。HTTP2会在客户端和服务器端使用”首部表“跟踪和存储发送的键值对，对于相同的数据，不在通过每次请求和响应发送。</p>
<p>首部表在HTTP2的连接存续期内始终存在，由客户端和服务器共同渐进地更新。</p>
<p>每个新首部键值对要么被追加到当前表的末尾，要么替换表中之前的值。</p>
<p><strong>服务器推送</strong></p>
<p>服务端可以主动推送其他资源给客户端，比如js文件和css文件，不需要等到解析到html的时候在发送这些资源。当然客户端也有权利选择是否接收，如果服务端推送的资源已经被浏览器缓存过，浏览器可以通过发送RST_STREAM帧来拒收，主动推送遵守同源策略，服务器不会随便推送第三方资源给客户端。</p>
<p><strong>多路复用</strong></p>
<p>http1想并发多个请求，必须使用多个TCP连接，同时会受到最大数量请求限制；</p>
<p>在http2中，同域名下所有通信都在单个连接上完成；单个连接可以承载任意数量的双向数据流；数据流以消息的形式发送，而消息又由一个或者多个帧组成，多个帧可以乱序发送，因为根据帧首部的流标识可以重新组装。</p>
<h4 id="http3-0" tabindex="-1"><a class="header-anchor" href="#http3-0" aria-hidden="true">#</a> HTTP3.0</h4>
<p>因为 HTTP/2 使⽤了多路复⽤，⼀般来说同⼀域名下只需要使⽤⼀个 TCP 连接。 由于多个数据流使</p>
<p>⽤同⼀个 TCP 连接，遵守同⼀个流量状态控制和拥塞控制。 只要⼀个数据流遭遇到拥塞，剩下的数据流就没法发出去，这样就导致了后⾯的所有数据都会被阻塞。HTTP/2 出现的这个问题是由于其使⽤ TCP协议的问题， 与它本身的实现其实并没有多⼤关系。</p>
<p>由于 TCP 本身存在的⼀些限制，Google 就开发了⼀个基于 UDP 协议的 QUIC 协议，并且使⽤在了</p>
<p>HTTP/3 上。 QUIC 协议在 UDP 协议上实现了多路复⽤、有序交付、重传等等功能</p>
<h4 id="队首阻塞" tabindex="-1"><a class="header-anchor" href="#队首阻塞" aria-hidden="true">#</a> 队首阻塞</h4>
<p>队首阻塞：就是需要排队，队首的事情没有处理完的时候，后⾯的⼈都要等着。</p>
<ol>
<li>http1.0的队首阻塞</li>
</ol>
<p>对于同⼀个tcp连接，所有的http1.0请求放入队列中，只有前⼀个请求的响应收到了，然后才</p>
<p>能发送下⼀个请求。</p>
<p>可见，http1.0的队首组塞发⽣在客户端。</p>
<ol>
<li>http1.1的队首阻塞</li>
</ol>
<p>对于同⼀个tcp连接，http1.1允许⼀次发送多个http1.1请求，也就是说，不必等前⼀个响应收</p>
<p>到，就可以发送下⼀个请求，这样就解决了http1.0的客户端的队首阻塞。但是，http1.1规</p>
<p>定，服务器端的响应的发送要根据请求被接收的顺序排队，也就是说，先接收到的请求的响应</p>
<p>也要先发送。这样造成的问题是，如果最先收到的请求的处理时间长的话，响应生成也慢，就</p>
<p>会阻塞已经生成了的响应的发送。也会造成队首阻塞。</p>
<p>可见，http1.1的队首阻塞发生在服务器端。</p>
<ol>
<li>http2是怎样解决队首阻塞的</li>
</ol>
<p>http2无论在客户端还是在服务器端都不需要排队，在同⼀个tcp连接上，有多个stream，由各</p>
<p>个stream发送和接收http请求，各个steam相互独立，互不阻塞。</p>
<p>只要tcp没有人在用那么就可以发送已经⽣成的request或者response的数据，在两端都不⽤等，</p>
<p>从而彻底解决了http协议层⾯的队首阻塞问题（但是tcp层仍存在问题）。</p>
...
<p>不在html中拉伸图片</p>
<h4 id="页面渲染性能优化" tabindex="-1"><a class="header-anchor" href="#页面渲染性能优化" aria-hidden="true">#</a> <a href="https://www.jianshu.com/p/d9c20eafa67e" target="_blank" rel="noopener noreferrer">页面渲染性能优化<OutboundLink/></a></h4>
<p>1.防止阻塞渲染</p>
...
<p>expires是HTTP1.0提供的，值为一个绝对时间，表示缓存新鲜日期</p>
<p>cache-control是HTTP1.1增加的，值是以秒为单位的最大新鲜时间</p>
<p>2.打开一个socket与目标IP地址、端口建立TCP连接，进行三次握手。依次通过传输层、网络层、数据链路层、物理层到达服务器。</p>
<p>3.建立了TCP连接后发送HTTP请求。</p>
<p>4.服务器接受请求并且解析，将请求转发到服务程序。</p>
<p>5.服务器检查请求头是否包含了缓存验证信息，如果验证缓存新鲜的话，返回304等对应状态码。</p>
<p>6.处理程序读取完整请求并准备HTTP响应，可能需要查询数据库等操作。</p>
<p>7.服务器将响应报文通过TCP连接发送回浏览器。</p>
<p>8.根据情况选择关闭TCP连接或者是保留重用，涉及到四次挥手。</p>
...
<p>DOM树描述了文档的内容，<html>元素是第一个标签也是文档树的根节点。树反映了不同标记之间的关系和层次结构，dom节点数量越多，构建dom树需要的时间就越长。</p>
...
<h4 id="缓存" tabindex="-1"><a class="header-anchor" href="#缓存" aria-hidden="true">#</a> <a href="http://verymuch.site/2018/10/09/web-browser-cache-strategy/" target="_blank" rel="noopener noreferrer">缓存<OutboundLink/></a></h4>
...
<p>问：<a href="https://www.cnblogs.com/SallyShan/p/13603221.html" target="_blank" rel="noopener noreferrer">资源更新了，但是时间没过期咋办<OutboundLink/></a></p>
...
<h4 id="get请求缓存问题" tabindex="-1"><a class="header-anchor" href="#get请求缓存问题" aria-hidden="true">#</a> get请求缓存问题</h4>
...
<p>4.客户端在GET请求的url中添加参数<code>@{@&quot;timestamp&quot;: @([[NSDate date] timeIntervalSince1970]}</code></p>
<p>6.用POSET替换GET请求</p>
...
<h4 id="cookie和session" tabindex="-1"><a class="header-anchor" href="#cookie和session" aria-hidden="true">#</a> <a href="https://blog.csdn.net/a754895/article/details/82632747" target="_blank" rel="noopener noreferrer">cookie和session<OutboundLink/></a></h4>
...
<p>解决跨域：</p>
<p><strong>使⽤ jsonp</strong> 来实现跨域请求，它的主要原理是通过动态构建 script 标签来实现跨域请求，因为<strong>浏览器对 script 标签的引⼊没有跨域的访问限制</strong> 。通过在请求的 url 后指定⼀个回调函数，然后服务器在返回数据的时候，构建⼀个 json 数据的包装，这个包装就是回调函数，然后返回给前端，前端接收到数据后，因为请求的是脚本⽂件，所以会直接执⾏，这样我们先前定义好的回调函数就可以被调⽤，从⽽实现了跨域请求的处理。这种⽅式只能⽤于 <strong>get</strong> 请求。</p>
...
<h4 id="http和tcp的区别和联系" tabindex="-1"><a class="header-anchor" href="#http和tcp的区别和联系" aria-hidden="true">#</a> HTTP和TCP的区别和联系</h4>
<p>tcp是在传输层的协议，定义的是数据传输和连接方式的规范，http是应用层协议，定义的是传输数据的内容和规范，HTTP协议中的数据是利用TCP传输的，支持HTTP就一定支持TCP。</p>
<h4 id="tcp和udp" tabindex="-1"><a class="header-anchor" href="#tcp和udp" aria-hidden="true">#</a> TCP和UDP</h4>
<p>TCP使用校验和、确认和重传机制来保证可靠传输，使用滑动窗口机制实现流量控制，通过动态改变窗口的大小实现拥塞控制。</p>
<p>TCP是面向连接的，因为有重传机制，所以可靠；因为通过SYN排序，所以有序，具有拥塞控制，但是传输速度慢；</p>
...
<p>如果 Web 应用程序没有部署足够的安全验证，那么，这些攻击很容易成功。浏览器无法探测到这些恶意脚本是不可信的，所以，这些脚本可以任意读取 cookie，session tokens，或者其它敏感的网站信息，或者让恶意脚本重写<a href="https://developer.mozilla.org/zh-CN/docs/Glossary/HTML" target="_blank" rel="noopener noreferrer">HTML<OutboundLink/></a>内容。</p>
...
<h5 id="跨站请求伪造csrf" tabindex="-1"><a class="header-anchor" href="#跨站请求伪造csrf" aria-hidden="true">#</a> <a href="https://tech.meituan.com/2018/10/11/fe-security-csrf.html" target="_blank" rel="noopener noreferrer">跨站请求伪造CSRF<OutboundLink/></a></h5>
...
<p>在前端向后端发起请求时，取出Cookie，并添加到URL的参数中（接上例<code>POST https://www.a.com/comment?csrfcookie=v8g9e4ksfhw</code>）。</p>
...
<p>Samesite=Lax</p>
<p>这种称为宽松模式，比 Strict 放宽了点限制：假如这个请求是这种请求（改变了当前页面或者打开了新页面）且同时是个GET请求，则这个Cookie可以作为第三方Cookie。</p>
<h4 id="性能优化比较新的内容-实践方向" tabindex="-1"><a class="header-anchor" href="#性能优化比较新的内容-实践方向" aria-hidden="true">#</a> <a href="https://sx54pm1tcw.feishu.cn/docs/doccnvBCsHDEpJTXbBkl9AZ24Ff#oxYdrn" target="_blank" rel="noopener noreferrer">性能优化比较新的内容（实践方向）<OutboundLink/></a></h4>
